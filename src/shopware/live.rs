//! Live-host refuse guards.
//!
//! `shopware sync apply` / `sync pull` refuse when the consumer looks live
//! unless `SHOPWARE_ALLOW_LIVE_RESTORE` is truthy.
//!
//! Standalone `shopware db import` uses the same live detection but allows an
//! explicit `--allow-live` (or the same env flag).

use super::env::{allow_live_restore, ShopEnv};
...
pub enum LivePolicy {
    /// Standalone import: refuse live unless `--allow-live` or env flag.
    DbImport { allow_live_flag: bool },
    /// `sync apply` / `sync pull`: refuse live unless `SHOPWARE_ALLOW_LIVE_RESTORE`.
    SyncRestore,
}
...
impl LiveSignals {
    pub fn from_shop(env: &ShopEnv) -> Self {
        Self {
            ...
            allow_env: allow_live_restore(env),
        }
    }
...
pub fn assert_not_live(signals: &LiveSignals, policy: LivePolicy) -> Result<Option<String>, Error> {
    ...
    match policy {
        LivePolicy::SyncRestore => Err(Error::fail(format!(
            "Refusing restore/sync on a live host (SHOPWARE_DEPLOY_ENV={deploy}, checkout={}, hostname={host}). Runtime sync is pull-only onto staging/playground/dev. Live restore is disaster recovery: set SHOPWARE_ALLOW_LIVE_RESTORE=1 to override.",
            signals.shop_basename
        ))),
        LivePolicy::DbImport { .. } => Err(Error::fail(format!(
            "Refusing database import on a live host (SHOPWARE_DEPLOY_ENV={deploy}, checkout={}, hostname={host}). Pass --allow-live or set SHOPWARE_ALLOW_LIVE_RESTORE=1. Casual imports are for staging/playground/dev; live is opt-in only.",
            signals.shop_basename
        ))),
    }
}
...
mod tests {
    ...
    fn restore_needs_env_even_if_import_flag() {
        ...
        let warn = assert_not_live(&s, LivePolicy::SyncRestore)
            .unwrap()
            .unwrap();
        assert!(warn.contains("SHOPWARE_ALLOW_LIVE_RESTORE"), "{warn}");
    }
